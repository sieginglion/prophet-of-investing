...
def get_invests_and_ratio(market):
    first_step = gspread.service_account('service-account.json').open('First Step').get_worksheet(0)
    invests = [row[0] for row in first_step.get({'Crypto': 'A11:A20', 'Stock': 'A35:A39'}[market])]
...
def filter_overvalued(market, symbols, ratio):
    ...
    for symbol in symbols:
        resp = r.get(f'https://us-central1-generated-armor-274023.cloudfunctions.net/CalcTargetPrice?market={ market }&symbol={ symbol }&ratio={ ratio }')
...
def find_worst_and_better(market, symbol_to_score):
    ...
    ranking_list = [symbol for symbol, _ in sorted(symbol_to_score.items(), key=lambda x: x[1], reverse=True)]
...
def notify(text):
    r.post(f'https://api.telegram.org/bot{ config.bot_token }/sendMessage', json={
        'chat_id': 1075192674,
        'text': text
    }).json()
